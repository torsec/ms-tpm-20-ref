...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
    ...
    TPM_HANDLE primarySeedHandle = TPM_RH_ENDORSEMENT;  // Usa il Primary Seed TPM
    TPM2B label = { .size = strlen("SPHINCS+ Key"), .buffer = "SPHINCS+ Key" };
    TPM2B contextU = { .size = 0, .buffer = {0} };
    TPM2B contextV = { .size = 0, .buffer = {0} };
    UINT32 sizeInBits = SEED_SIZE * 8;  // 768 bit
    UINT32 counter = 0;
    UINT16 blocks = 1;  // Può essere regolato
    BYTE keyStream[SEED_SIZE] = {0};

    UINT16 generated = CryptKDFa(
        hashAlg, (TPM2B*)&primarySeedHandle, &label, &contextU, &contextV,
        sizeInBits, keyStream, &counter, blocks
    );
    
    for (size_t i = 0; i < SEED_SIZE; i++) {
        printf("%02X", keyStream[i]);  // Stampa ogni byte in esadecimale
    }
    printf("\n");
    printf("Arrivato a crypto_sign_keypair\n");
    crypto_sign_seed_keypair(pk, sk, keyStream);
